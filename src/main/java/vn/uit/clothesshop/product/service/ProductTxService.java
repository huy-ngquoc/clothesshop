...
import vn.uit.clothesshop.product.service.ProductService.CreationError;
import vn.uit.clothesshop.product.service.ProductService.DeletionError;
import vn.uit.clothesshop.product.service.ProductService.UpdateError;
...
public class ProductTxService {
    ...
    public ProductTxService(
            ...
            @NotNull final CategoryAccess categoryAccess) {
    ...
    long handleCreateProduct(
        ...
        final var categoryUpdated = this.categoryAccess.increaseProductCount(categoryId, 1);
        if (!categoryUpdated) {
            throw new CreationException(CreationError.CANNOT_UPDATE_CATEGORY);
        }

        final var category = this.categoryAccess.findById(categoryId).orElse(null);
    ...
    public void handleUpdateProduct(
        ...
        if (oldCategoryId != newCategoryId) {
            ...
            final var newCategory = this.categoryAccess.findById(newCategoryId).orElse(null);
    ...
    void deleteProductById(final long id)
        ...
        final var categoryUpdated = this.categoryAccess.decreaseProductCount(product.getCategoryId(), 0);
        if (!categoryUpdated) {
